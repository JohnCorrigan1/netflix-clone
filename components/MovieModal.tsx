...
const MovieModal: React.FC<{ open: boolean; movie: FeaturedMovie | null }> = (
  props
) => {
  ...
  return (
    <>
      ...
      <div className="bg-zinc-900 text-zinc-200 flex flex-col items-center rounded-xl fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-5/6 max-w-3xl overflow-y-scroll scrollbar-hide h-5/6 z-[2001]">
        <div
          className="hover:bg-zinc-400 rounded-full bg-opacity-50 active:scale-95 absolute top-3 right-3"
          onClick={closeHandler}
        >
        ...
        <div className="flex w-full mt-1 ml-5 gap-5">
          ...
          {props.movie?.mediaType && (
            <h2 className=" ml-16 font-semibold text-lg">
              {props.movie?.mediaType}
            </h2>
          )}
          <div
            onClick={addLibraryHandler}
            className="hover:cursor-pointer active:scale-90 p-1 bg-zinc-800 rounded-2xl flex items-centers pr-2 font-semibold hover:bg-zinc-500"
          >
            <Image
              className=""
              src="/add.svg"
